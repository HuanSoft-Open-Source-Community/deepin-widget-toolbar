...
- **⚙️ Panel Settings**: the settings popup (right-click → Settings, or via the tray menu) holds show-panel and pin-to-top switches plus a *card transparent mode* toggle: when on, every widget card uses a translucent overlay background (default off), independent of each widget's own transparent-background setting
...
- **💾 State Persistence**: `visible`, `pinned` and the card-transparent mode are persisted via DConfig and restored on restart; widget instances are stored in `~/.local/share/org.deepin.ds.widgettoolbar/installed.json`, and per-instance settings are stored under each widget's data directory
