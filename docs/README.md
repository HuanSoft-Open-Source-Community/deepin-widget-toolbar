...
- **🚪 Panel Avoidance**: When another DDE surface *actually overlaps* the right-edge sidebar — notification pop-ups, the notification center, the control center, the power/shutdown dialog, the clipboard panel, or a tray quick-panel / expanded mini-card — the panel likewise hides for the duration and returns the instant they close or move clear (mirroring the multitask avoidance, sharing its pop-up cleanup); a mini-card that opens clear of the sidebar no longer hides it, while unreadable geometry falls back to avoiding; X11-only, detection reads each window's WM_NAME / WM_CLASS plus its root-absolute rectangle (the sidebar's own rect is folded from its global position and screen scale factor)
