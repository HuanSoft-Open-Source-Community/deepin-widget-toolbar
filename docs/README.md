...
- **⚙️ Panel Settings**: the settings popup (right-click → Settings, or via the tray menu) holds show-panel, pin-to-top, *card transparent mode* and *card names* switches: transparent mode makes every widget card use a translucent overlay background (default off), independent of each widget's own transparent-background setting
...
- **💾 State Persistence**: `visible`, `pinned`, the card-transparent mode and `showCardNames` are persisted via DConfig and restored on restart; widget instances are stored in `~/.local/share/org.deepin.ds.widgettoolbar/installed.json`, and per-instance settings are stored under each widget's data directory
...
│       ├── SettingsDialog.qml  # settings popup (show panel / pin to top / card transparent mode)
