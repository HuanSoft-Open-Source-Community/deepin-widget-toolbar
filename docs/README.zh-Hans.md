...
- **🚪 面板避让**：当其他 DDE 面板在几何上*确实与*右侧栏叠合——通知弹出、通知中心、控制中心、电源/注销对话框、剪贴板面板，或托盘快捷面板与展开小卡——面板同样临时隐藏、待其关闭或移开即时回归（与多任务视图避让同款，共用弹窗收起逻辑）；展开在侧栏之外的小卡不再误隐，几何不可知时退化为照旧避让；仅 X11 生效，依各窗口的 WM_NAME / WM_CLASS 与其在 root 上的绝对矩形识别（侧栏自身矩形由全局位置与本屏缩放比折算）
