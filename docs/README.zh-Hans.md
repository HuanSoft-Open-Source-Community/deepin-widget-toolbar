...
- **🖱️ 拖放与整理**：长按可把小组件拖到任意格子（含每行非首列）；位置持久保持、不做强制补位；拖拽过程中被占用的小组件会实时双向让位（多个组件联动、带动画），留在被拖卡片原位的半透明虚影会跟随预测落点逐格移动（弹性动画），松手前即可看到确切落点，移开或取消则回弹；上浮预览当帧出现在指针处、不做飞入；左下角"整理"按钮按左上优先压实
...
- **⚙️ 面板设置**："设置"弹窗（右键 → 设置，或经托盘菜单）提供显示面板、置顶、*卡片透明模式*、*卡片名称显示*与*调试模式*五项开关：卡片透明模式开启时所有小组件卡片使用半透明叠层底（默认关闭），与每个小组件自身的透明背景开关相互独立；调试模式（默认关闭）是面板能否向磁盘写日志的唯一闸门
...
- **💾 状态持久化**：`visible`、`pinned`、卡片透明模式、`showCardNames` 与 `debugMode` 经 DConfig 持久化，重启后恢复；小组件实例清单存于 `~/.local/share/org.deepin.ds.widgettoolbar/installed.json`，按实例配置存于对应小组件数据目录
- **🐞 调试日志**：开启*调试模式*后，面板把带时间戳的日志写入 `~/.cache/logs/deepin-widget-toolbar/`，覆盖面板显隐与置顶变更、两条避让路径、多任务视图信号、小组件扫描与增删实例，以及全部错误与降级分支；单个文件满 2500 行即另起一个新时间戳文件。默认关闭，关闭时一条都不写——对账心跳这类高频点刻意不落日志，只记真实跃变与故障
...
2. 点击面板标题栏的图钉按钮，切换置顶/置底：
   ...
   - **置底**：普通窗口可以覆盖面板。
3. 显隐与置顶状态在重启后保持。
4. 需要排查面板问题时，打开**设置 → 启用调试日志**。日志随即写入
   `~/.cache/logs/deepin-widget-toolbar/`，每次运行一个文件，文件名形如
   `yyyyMMdd_HHmmss.log`，每满 2500 行自动另起一个新的时间戳文件。关掉开关即
   完全停止写入；该目录属普通用户缓存，随时可删。注意 `~/.cache` 是按用户的，
   若面板曾在别的账号下运行，那份日志在对方的缓存目录里，不在这里。
...
│       ├── SettingsDialog.qml  # 设置弹出面板（显示面板/置顶/卡片透明模式/调试模式）
