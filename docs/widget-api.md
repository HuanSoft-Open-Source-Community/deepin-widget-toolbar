...
时钟/世界时间表盘不再绘制表盘底色。宿主设置面板据此隐藏透明下不再生效的颜色设置行
（卡片底/区域底色及被主题自适应文字接管的文字色），文字与指针等内容色设置行保留；
全局卡片透明模式开启时，内置小组件的设置面板同时隐藏其 `transparentBackground`
开关本身（任一开关生效即透明，单卡开关无视觉效果；第三方组件未必响应全局
模式，其开关保留）。
...
- 尺寸切换：宿主以约 220ms 缓动平滑缩放卡片渲染框，小组件内容随框逐帧重排；小组件只需跟随传入的 width/height 布局。
