...
- **內置**：時鐘（數字/指針）、曆、系統之監（凡尺寸皆可擇 CPU/內存/磁盤/GPU/NPU 之顯，可示所啟之全部；二乘二單列，四乘二與四乘四默雙列；刷新可擇一秒、二秒、五秒而默五秒；後台按需采樣；進度條輕而無動畫）、便箋（每例各存其文）、世界之時（數字或隨尺寸而變之指針表盤網格，時區與地區之名隨控制中心時間之設），及可擇字體顏色之端闈樂部歌詞。時鐘與世界之時並預載時刻（默啟）：諸表共享宿主一己之秒鐘，表盤靜者先繪，指針以 GPU 旋之，多盤並動而不滯。凡內置組件皆可逐色而設（一默色加自選），並有透明背景之闕（默閉），色不合法則退默
- **開宗**：manifest（`sizes` + `settings`）+ 例之上下文（`dataDir`/`instanceId`/`widgetConfig`）+ 宿主之能（`FileIO`/`SystemInfo`/`Lyrics`/`ClockTime`），詳見 [widget-api.md](widget-api.md)
