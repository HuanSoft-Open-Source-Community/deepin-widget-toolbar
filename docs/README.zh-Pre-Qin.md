...
- **內置**：時鐘（數字/指針）、曆、系統之監（凡尺寸皆可擇 CPU/內存/磁盤/GPU/NPU 之顯，可示所啟之全部；二乘二單列，四乘二與四乘四默雙列；刷新可擇一秒、二秒、五秒而默五秒；後台按需采樣；進度條輕而無動畫）、便箋（每例各存其文）、世界之時（數字或隨尺寸而變之指針表盤網格，時區與地區之名隨控制中心時間之設），及可擇字體顏色之端闈樂部歌詞
